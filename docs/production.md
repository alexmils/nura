...
8. **2026-09-22:** Coolify UI showed **failed** on every deploy after ~22:50 UTC even though the site stayed up. Cause: `nura-ensure-docker-watch` (45s debounce) ran `restore-app.sh` mid Coolify health wait (60s `custom_healthcheck_found` / Dockerfile HEALTHCHECK), deleted the container Coolify was `docker inspect`-ing (`no such object`), then recreate with `date +%s%3N` naming. Fix on VPS: Coolify `health_check_start_period=15`, `custom_healthcheck_found=false`, watch debounce **90s** + skip while deploy `queued`/`in_progress`; ensure/restore lock scripts keep those settings (no longer reset to 60).
