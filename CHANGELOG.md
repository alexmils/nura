...
- [internal] Server-side conversion tracking (`docs/conversions.md`): GA4 Measurement Protocol, Meta Conversions API, and Google Ads offline click upload now report the **first real charge** from the Stripe `invoice.paid` webhook, which nothing in the browser could see under a 7-day trial; first-touch `gclid`/`gbraid`/`wbraid`/`fbclid`/GA4 `client_id` captured into `nura_attr` and persisted to `user_attribution` at signup and checkout; `conversion_dispatches` makes each `(channel, invoice)` deliver at most once
- [internal] Client tags now mirror every Meta event to GA4 (`sign_up`, `begin_checkout`, `start_trial`, `start_subscription`, `generate_lead`) with `value`/`currency`/`items`, gated separately by the analytics vs marketing choice — GA4 previously received no custom events at all; the browser never sends GA4 `purchase` so the delayed charge cannot be double-counted
...
- **Guide** now steps through the controls on a phone too: the session list slides away when the tour moves on to the session, and the tour opens its own empty session instead of touching the one you were in, so nothing interrupts you with a close-session prompt
