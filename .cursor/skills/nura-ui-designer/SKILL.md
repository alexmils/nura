...
- **After pause:** Memory sets (`HomeMemorySets`) — left: mid-session chat (not a cold ask). Agent continues a thread; user mentions a past note; **Memory loading** then **Memory processing**; agent replies with the stored detail. Right: kicker **Memory**; H2 **Notes you keep. The agent sees them only when the set is on.** (`LetterRevealHeading`); three full-sentence points (what a set is / switch on in AI agent-guided / only enabled notes). No infinite-memory claim.
